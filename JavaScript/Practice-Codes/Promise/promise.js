...
function createOrder() {
  ...
  const pr = new Promise(function (resolve, reject) {
    ...
    if (!validateCart()) {
      reject(new Error("Cart is not valid")); // new Error is a way to define the error message
    } else {
      const orderId = "12345";
...
createOrder()
  ...
  .then((paymentDataRecievedFromPrevious) => {
    console.log("Payment Data is :", paymentDataRecievedFromPrevious);
  })

  .catch((err) => {
    console.log("Error is", err.message); // e.message is way to catch the error message
  });
